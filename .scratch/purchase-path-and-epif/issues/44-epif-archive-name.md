...
- [ ] `log_writer.save_epif` refuses to replace an existing file: if the destination exists it raises `FileExistsError` instead of calling `os.replace`. A test writes a file, calls `save_epif` with the same name, asserts the error, and asserts the original bytes are unchanged.
